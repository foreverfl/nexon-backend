...
- 인증 및 권한 처리 흐름
  ...
  - gateway 서버 내 엔드포인트별 authGuard 및 roleGuard 설정
- 이벤트 → 보상 흐름 설명
...
- `localhost:3000` ~ `3002` 포트가 사용 중이지 않아야 합니다.
- Redis (`6379`), MongoDB (`27017`), Kafka (`9092`)의 기본 포트가 사용 중이지 않아야 합니다.
- `.env` 파일은 `.gitignore`에 포함하지 않고, 실제 프로젝트 내에 함께 포함되어 있습니다. 별도로 생성할 필요는 없습니다.
...
- `/test` 폴더에 있는 데이터를 토대로 api를 테스트할 수 있습니다.
  - `/test/http` 폴더에 있는 요청 예시를 통해서 swagger에 등록된 api를 테스트할 수 있습니다.
  - `/test/grpc` 폴더에 있는 요청 예시를 통해서 grpcurl을 사용하여 grpc api를 테스트할 수 있습니다. markdown 파일에는 각 명령어를 넣어두었고, 필요한 proto 파일은 같은 경로에 두었습니다. 마크다운 명령어 내에서 import-path를 수정하여 사용하시면 됩니다.
